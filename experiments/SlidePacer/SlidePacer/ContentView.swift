...
enum SourceExtractor {
    ...
    static func brief(_ selection: ExtractiveSelection, points: [SourcePoint], roleHint: String?, source: SlideInput? = nil) throws -> EditorialBrief {
        ...
        guard (1...5).contains(selection.priority), !selection.coreIDs.isEmpty, selection.coreIDs.count <= 2, selection.detailIDs.count <= 2,
        ...
        if let comparable = comparisonIDs(points, roleHint: roleHint) {
            guard Set(selection.coreIDs).count == 2, Set(selection.coreIDs).isSubset(of: Set(comparable)) else {
...
struct ContentView: View {
    ...
    // バックエンド選択
    @State private var backend: Backend = .ollama
    ...
    enum Backend: String, CaseIterable, Identifiable {
        case foundationModels = "FoundationModels (4k, 端末内)"
        case ollama = "Ollama (ローカルサーバ)"
    ...
    var body: some View {
        NavigationStack {
            Form {
                ...
                promptTuningSection
                generationOptionsSection.disabled(isGenerating)
                actionSection
    ...
    private var availabilitySection: some View {
        Section("モデル状態") {
            if backend == .ollama {
                Label("Ollama：\(ollamaModel)", systemImage: "desktopcomputer")
                Text("接続先：\(ollamaBaseURL)\n接続とモデルの有無は分析時に確認します。")
                    .font(.caption).foregroundStyle(.secondary)
            } else {
                switch SystemLanguageModel.default.availability {
                case .available:
                    Label("オンデバイスモデル利用可能", systemImage: "checkmark.circle.fill").foregroundStyle(.green)
                case .unavailable(let reason):
                    Label("利用不可: \(String(describing: reason))", systemImage: "exclamationmark.triangle.fill").foregroundStyle(.orange)
                @unknown default: Text("モデルの状態を取得できません")
                }
            }
        }
    }
    ...
    private var backendSection: some View {
        Section {
            ...
            if backend == .ollama {
                ...
                Text("Qwen3系は思考モードを無効にします。Qwen3.5ではtop_p等の推奨設定を併用します。")
                    .font(.caption).foregroundStyle(.secondary)
                TextField("モデル名 (例: qwen3.5:9b)", text: $ollamaModel)
                    .textFieldStyle(.roundedBorder)
                    .font(.footnote.monospaced())
                HStack {
                    Text("num_ctx")
                    Spacer()
                    Text("\(Int(ollamaNumCtx))")
                        .foregroundStyle(.secondary)
                        .font(.callout.monospacedDigit())
                }
                Slider(value: $ollamaNumCtx, in: 2048...32768, step: 1024)
                Text("Ollama側で確保するcontextトークン数。大きいほど長文可、メモリを使う。")
                    .font(.caption)
                    .foregroundStyle(.secondary)
            }
        } header: {
    ...
    private var generationOptionsSection: some View {
        Section {
            ...
            Text("低いほど決定的、高いほど多様。0.0〜2.0")
                .font(.caption)
                .foregroundStyle(.secondary)

            if backend == .foundationModels {
                Toggle("プロンプトにスキーマを含める (includeSchemaInPrompt)", isOn: $includeSchemaInPrompt).font(.footnote)
            }

            Toggle("maximumResponseTokens を指定", isOn: $useMaxTokens)
                .font(.footnote)
            if useMaxTokens {
                HStack {
                    Text("max tokens")
    ...
    private var actionSection: some View {
        Section {
            ...
            .disabled(isGenerating || slides.isEmpty)
    ...
    private func runAnalysis() async {
        ...
        do {
            ...
            if backend == .ollama {
                ollamaBaseURL = ollamaBaseURL.trimmingCharacters(in: .whitespacesAndNewlines)
                ollamaModel = ollamaModel.trimmingCharacters(in: .whitespacesAndNewlines)
                _ = try AnalysisInputValidation.ollamaEndpoint(ollamaBaseURL)
                guard !ollamaModel.isEmpty else { throw AnalysisInputError.invalidInput("Ollamaのモデル名を入力してください。") }
            }
            generationStatus = "資料全体の主張と重要度を比較中"
            let overviewPrompt = DeckDirector.prompt(slides: slides, theme: theme, goal: presentationGoal, audience: audience, outlineLimit: backend == .foundationModels ? 2400 : max(1200, min(12000, Int(ollamaNumCtx) * 2 / 3)))
            lastPrompt = "--- 全体構成 ---\n" + overviewPrompt
            let directionKey = [backend.rawValue, ollamaBaseURL, ollamaModel, String(temperature), String(ollamaNumCtx), DeckDirector.instructions, overviewPrompt].joined(separator: "\u{001F}")
            let direction: DeckDirection
            if let cached = directionCache[directionKey] { direction = cached }
            else if backend == .ollama {
                direction = try await OllamaClient.generateDirection(baseURL: ollamaBaseURL, model: ollamaModel, prompt: overviewPrompt, count: slides.count, temperature: temperature, context: Int(ollamaNumCtx))
            } else {
                let session = LanguageModelSession(instructions: DeckDirector.instructions)
                direction = try await session.respond(to: overviewPrompt, generating: DeckDirection.self, options: GenerationOptions(temperature: temperature)).content
            }
            try Task.checkCancellation()
            ...
            for index in slides.indices {
                try Task.checkCancellation()
                generationStatus = "内容を要約中：\(index + 1)/\(slides.count)"
                let points = SourceExtractor.points(slides[index], index: index + 1)
                guard !points.isEmpty else { throw OllamaError.decodeFailed("ページ\(index + 1)には本文・ノートがありません。画像の説明をノートに入力してください。") }
                lastSources += points
                let prompt = buildPrompt(page: index) + DeckDirector.pageContext(index + 1, direction: direction, slides: slides)
                lastPrompt += "\n--- ページ\(index + 1)への入力 ---\n" + prompt
                // 時間だけを変えた再配分では、同じ内容・設定の要約を再利用する。
                let key = [backend.rawValue, ollamaBaseURL, ollamaModel, String(temperature),
                    String(ollamaNumCtx), String(useMaxTokens), String(maxTokens),
                    String(includeSchemaInPrompt), instructions, prompt].joined(separator: "\u{001F}")
                let selection: ExtractiveSelection
                if let cached = briefCache[key] { selection = cached }
                else {
                    switch backend {
                    case .foundationModels: selection = try await runFoundationModels(prompt: prompt)
                    case .ollama:
                        selection = try await OllamaClient.generateSelection(
                            baseURL: ollamaBaseURL, model: ollamaModel, system: instructions,
                            prompt: prompt, validIDs: points.map(\.id), comparisonIDs: SourceExtractor.comparisonIDs(points, roleHint: SourceExtractor.roleHint(slides[index], index: index + 1, count: slides.count)), temperature: temperature,
                            maxPredictTokens: useMaxTokens ? Int(maxTokens) : 768, numContext: Int(ollamaNumCtx))
                    }

                }
                try Task.checkCancellation()
                lastSelections.append(PageSelection(slideIndex: index + 1, selection: selection))
                let brief = try SourceExtractor.brief(selection, points: points,
                    roleHint: SourceExtractor.roleHint(slides[index], index: index + 1, count: slides.count), source: slides[index])
                briefCache[key] = selection
                lastBriefs.append(PageBrief(slideIndex: index + 1, brief: DeckDirector.apply(brief, page: index + 1, direction: direction, slides: slides)))
            }
            try Task.checkCancellation()
            generationStatus = "要約から時間配分を計算中"
            plan = try EditorialPlanner.assemble(lastBriefs, slideCount: slides.count, budget: totalSeconds)
        } catch {
            if Task.isCancelled || error is CancellationError {
                analysisNotice = "分析を中止しました。設定を変更して再実行できます。"
                return
            }
    ...
    private func runFoundationModels(prompt: String) async throws -> ExtractiveSelection {
        let options: GenerationOptions = {
            if useMaxTokens {
                return GenerationOptions(temperature: temperature, maximumResponseTokens: Int(maxTokens))
            } else {
        ...
        let response = try await session.respond(
            to: prompt,
            generating: ExtractiveSelection.self,
    ...
    private func buildReportJSON(plan: WeightingPlan?) -> Data? {
        ...
        let params = AnalysisReport.Params(
            ...
            ollamaBaseURL: backend == .ollama ? ollamaBaseURL : nil,
            ollamaModel: backend == .ollama ? ollamaModel : nil,
            ollamaNumCtx: backend == .ollama ? Int(ollamaNumCtx) : nil,
            temperature: temperature,
            includeSchemaInPrompt: includeSchemaInPrompt,
            maximumResponseTokens: useMaxTokens ? Int(maxTokens) : (backend == .ollama ? 768 : nil),
            ollamaSampling: backend == .ollama && OllamaModelProfile.usesNonThinkingSampling(ollamaModel)
                ? ["top_p": 0.8, "top_k": 20, "min_p": 0, "presence_penalty": 1.5, "repeat_penalty": 1] : nil,
            ollamaThinkingEnabled: backend == .ollama && OllamaModelProfile.supportsThinkingSwitch(ollamaModel) ? false : nil
        )
