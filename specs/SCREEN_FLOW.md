...
2026-09-15更新：下部スライド直接操作、PPTXノート、タイマー・通知、カメラ・結果は部分実装済み。以下の表は目標遷移で全画面の完成を意味しない。現状は[STATUS](../docs/STATUS.md)、見本は[SCREEN_REVIEW](../docs/SCREEN_REVIEW.md)。QRはPR #71、iPhone簡素化はPR #70でmain統合済み。Mac準備再編は未マージPR #67と区別する。以下のP02bの手動コード等は全て同じ仕様で実装したわけではなく、現行QRは[QR_CONNECTION](../docs/QR_CONNECTION.md)を参照。
...
この設計の全遷移を通した実機受入は未完了。部分実装の自動テスト/Simulator確認は[STATUS](../docs/STATUS.md)から各記録を参照。図は全例外の個別分岐を省略しているため、画面表・例外表を正本とする。
