# キャラクター案内・翻訳・分析の画面設計（設計と部分実装）

実装状態の追記：カメラ部分は[CAMERA_ANALYSIS.md](CAMERA_ANALYSIS.md)に実装範囲を記録した。Mac/iPhoneの既存画面に入口と状態表示を追加し、B03/B05/R03/R05の補助パネルを接続した。iPhone音声の発表関連付け・分析は[AUDIO_CAPTURE](../docs/AUDIO_CAPTURE.md)、実測時間と当端末カメラ結果は[RESULTS_DEVELOPMENT](../docs/RESULTS_DEVELOPMENT.md)に実装範囲を記録した。笑顔判定・翻訳本体統合・統一タイムラインと改善候補は残件。MCP振り返りはPR #58でmainへ統合済み（2026-09-15、`66588fe`）。実ChatGPT・実機の検証範囲は[MCP_INTEGRATION](../docs/MCP_INTEGRATION.md)で区別する。下記の画像は企画画像のままである。
...
後続実装 #86：準備アプリと本体の同一資料を照合し、原文から選んだ要点案を本体で確認・採用・元へ戻す導線を追加。[準備手順](../docs/PREPARATION_NOTES.md)。採用後は現在ページのiPhone原稿へ表示。ChatGPT MCPの明示共有経路であり、下記ローカル専用案・翻訳・個別提案編集・本体への分析画面埋込みの完成とは区別する。

以下は当初のローカル処理案。PR #58には利用者の明示共有でChatGPTへ渡すMCP経路があり、処理場所・操作・保存は[MCP_INTEGRATION](../docs/MCP_INTEGRATION.md)を参照する。両経路を混ぜて「外部送信なし」と案内しない。
...
撮影してよい範囲、必要な同意、保存/共有の扱いを事前に確認する。当初は生音声・映像を保存しない案だった。現実装はカメラ集計を端末内メモリに保持し、音声は明示録音の一時WAVを保持して再送/聞き直しに使う。破棄・次回起動で削除する詳細は[AUDIO_CAPTURE](../docs/AUDIO_CAPTURE.md)。永久保存の合意ではない。ローカルで処理するのは目標であり、モデル選定前に全分析の完全オフライン動作を保証しない。バックグラウンドでの撮影継続も保証しない。

## 終了後: 結果と改善

改善提案の目的・入力・具体例は[AI_COACHING](AI_COACHING.md)を正本にする。R01の改善候補1件から、対象・観測した事実・改善の仮説・具体的な修正・次の練習での確認へつなぐ設計。根拠が不足すれば提案を埋めず、分析できた範囲を示す。対象原稿/録音への復帰、提案の採用記録、前回比較は追加実装案として区別する。

カメラ補助パネルと実タイマーに紐付く終了サマリーの実装範囲は[RESULTS_DEVELOPMENT](../docs/RESULTS_DEVELOPMENT.md)を参照。個別音声結果とカメラの撮影相対時間帯は実装済み。共通サマリーの音声・改善候補・同期した時間帯グラフ・実機受け入れは未完了。
...
本番開始後の検証: 案内スキップと再表示、分析未導入で発表まで到達、原稿変更の取消、音声なしと沈黙の区別、カメラ対象の誤表示防止、入力切断と背景復帰、結果準備中でも次の発表へ進めること。個別実装には自動回帰・ビルド記録があるが、上記一巡の実機受入・精度・速度・全端末対応の保証は未完了。
