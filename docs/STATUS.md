...
準備・練習・音声・AI振り返りをつなぐ統合版を実装しています。ホーム「練習する」、音声接続情報の自動共有、メイン画面の録音・分析入口、資料AI整理の本体内表示を追加。[操作・統合元・検証範囲](INTEGRATED_PRACTICE.md)。以下のスナップショットは履歴であり、最新統合状態はIssue #92とそのPRで確認してください。

## Issue #83の追加差分（2026-09-15）

この変更の基準mainは`66588fe`（PR #58/#72統合済み）。以下は#83の実装差分であり、上記以外の古いスナップショットの全項目を再監査したものではない。マージ・CIの現在値は[Issue #83](https://github.com/MinobeRyo/kanpeki/issues/83)から確認する。

- フィラーと低音量区間を同じ録音の時刻順候補にし、結果から2ボタンの聞き直し詳細を開く。文字起こし未計測でも低音量を再生可能。
- 録音変更・再分析時の候補無効化、再生/録音/分析のID照合で古いTimer・中断処理を拒否。検出/API/AI契約や異なる時計の同期は変更しない。
- `AudioReviewChecks`をcore実行経路へ登録。合成候補・境界・未計測・重複・旧再生拒否を検証。実機の音声出力/中断は未確認、統一タイムラインは#17、精度・会場受入は#39に残す。[仕様](../specs/FILLER_CHAPTERS.md)。

## 以前の全体監査
## 本PRの追加差分：事前の要点案の採用（#86）

Macの同一資料を準備アプリへ明示読込し、分析済み要点案を本体で原文比較→採用→現在ページのiPhone原稿表示→元に戻す導線を追加。[準備手順・制限](PREPARATION_NOTES.md)。新しい本文生成・翻訳・共通ホームへの準備アプリ埋込みではない。接続前にMacだけで準備可能。分析なしの発表を維持。実ChatGPT返却・実PowerPoint/iPhoneの一巡・配布は未検証。下記は記載時点のmainスナップショットで、本PRマージや現在値を保証しない。
...
2026-09-15: 明示的な画面選択操作ではScreenCaptureKitへ取得を依頼し、事前の権限判定だけで打ち切らないよう修正。起動時の非対話取得は権限なしなら行わない。取得成功を許可状態に反映し、権限拒否時のみシステム設定/オンなら⌘Qで終了して再起動する案内を表示する。一般的な取得失敗を権限不足と断定しない。権限設定の書換えや自動終了はしない。APIを差し替えた回帰検証と実端末の許可反映は別であり、実端末の症状解消は未確認。

## QR表示の安定化（#97）

同一接続情報のJSONを固定順に符号化し、再描画でQRが揺れないよう修正。再表示は有効な待受を維持し、明示更新だけ鍵を再発行する。Mac/iPhoneビルドとcore検証は成功。会場環境のローカルTLS試験は変更前mainでも接続失敗し、実iPhoneの接続停止は未解消確認。端末間隔離・ネットワーク経路・Mac側許可などの切り分けを継続する。

## 画面共有の開始直前確認（#94・追加修正）

2026-09-15: 1044.1.0の実ログでは画面収録はTCC Allowだが、選択ウィンドウのcontent filter生成が失敗（CoreGraphicsErrorDomain 1003）。失敗時のウィンドウIDは後の照会では消滅しており、現在のPowerPointウィンドウを使った独立診断では開始/停止が成功。ただし失敗時点の消滅は未確定。
開始直前にScreenCaptureKitから一覧を再取得し、同じIDと所有プロセスの画面だけを使う。消えた対象や開始失敗後は準備完了のままにせず選択へ戻す。別画面・画面全体へ自動変更しない。エラーのdomain/codeを案内とOSログへ残す。検証はIssue #94の追加PRを参照。TestFlightアプリでの映像転送・スライド送り成功は未確認。
