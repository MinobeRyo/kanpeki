...
PowerPoint描画の取得、PPTXノート、Mac/iPhone通信の既存コードを含む。最新デザインの全画面や分析機能が本体へ実装済みとは限らない。SlidePacerのMCP分析とMac状態の共有は #33 で統合中です。準備アプリの共通ホームへの埋め込みと事前の資料分析結果のiPhone表示は別作業です。音声・カメラ・資料・時間をまとめたMCP振り返りとMac/iPhoneへの根拠付き結果表示を追加しました。[MCP連携](MCP_INTEGRATION.md)を参照してください。
