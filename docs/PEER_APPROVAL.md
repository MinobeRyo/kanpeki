...
MultipeerConnectivityの暗号化・Macの明示承認・操作用iPhone1台の構成を維持する。QR/WebSocketへは移行しない。
...
これは端末名を本人確認の証明にするものではない。人間は自分のiPhoneで接続操作したことを確認して承認する。短寿命照合コード・QR・ネットワーク越しの到達性・WebSocket移行は未実装で、#19は完了扱いにしない。
