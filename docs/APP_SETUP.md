...
4. Mac/iPhoneの画面を開くと待機・検索する。見つかったMacを選択し、見つからない場合は「QRでつなぐ」を使う（[QR手順](QR_CONNECTION.md)）。Macで本人の端末を確認して許可する。まず同じ信頼できるネットワークで試す。
...
PR #70の簡素化、PR #71のQR直接接続、PR #64の1台承認はmain統合済み。PR #67のMac準備カード・統合開始は未マージ。通常通信は暗号化MultipeerConnectivity、QRは直接TLS接続で、WebSocketへの全面移行ではない。

## 任意の分析

### PR #67のMac導線（統合・配布状態はPRで確認）

上記の個別「共有開始」「PowerPoint操作有効化」は、#67の開発版では準備パネルの「発表を始める」へ統合される。資料・共有画面・接続方法・時間を確認し、共有とPowerPointの現在画像確認が成功した後だけ時計が開始する。起動時の一覧更新は許可済みの場合のみ。Macだけで使う場合は接続方法で明示して適用する。
途中取消・接続変更・取得失敗で自動開始しない。発表中の共有/PPT失敗は「その他」→準備パネル→「共有・操作を確認し直す」から選択・確認して復旧し、時計はリセットしない。「その他」から終了確認にも進める。音声分析・QRは引き続き上部から開く。
詳細は[Mac UI実装記録](MAC_UI_IMPLEMENTATION.md)。PPTXを選ぶだけで内容をプレビューする機能と逐次準備は後続PR #81であり、#67単体には含めない。

- カメラ：Macの準備パネル/iPhone「…」の設定・結果から対象/前後を確認して明示開始。閉じても計測を続ける。端末内メモリのみ。[カメラ](CAMERA_DEVELOPMENT.md)。
- 音声：メニュー→その他→確認・接続の試験入口、または同じ確認・接続メニューの「発表の音声」から明示録音。カメラを停止する。同発表の終了/切断/背景で停止し、取得済み録音を明示送信。[音声手順](AUDIO_CAPTURE.md)。Mac上部「音声分析」でモデルを準備して受信を開始し、URLとコードをiPhoneへ入力する。[Mac音声ホストの手順](MAC_AUDIO.md)、実装・統合と配布状況は[PR #72](https://github.com/MinobeRyo/kanpeki/pull/72)を参照。
