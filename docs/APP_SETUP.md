...
- 音声：メニュー→その他→確認・接続の試験入口、または同じ確認・接続メニューの「発表の音声」から明示録音。カメラを停止する。同発表の終了/切断/背景で停止し、取得済み録音を明示送信。[音声手順](AUDIO_CAPTURE.md)。mainはMacのPython音声サーバーを別起動する。ネイティブ音声ホストは未マージPR #72。
