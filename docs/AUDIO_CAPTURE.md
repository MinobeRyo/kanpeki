...
5. 話速、フィラー候補、低音量区間、文字起こしを確認する。「気になる箇所を聞き直す」から共通の時刻順候補をスライダーで選び、保持中の録音の前後2秒を再生できる。フィラー未計測でも低音量区間を聞ける。録音を破棄すると再生できない。

未検証：実iPhoneの権限・録音品質・LAN通信、会場音声精度、長時間録音。実機での確認は #39 に記録する。

聞き直し詳細の操作は戻ると再生/停止の2ボタン。低音量を失敗と断定せず、未計測と0件を区別する。分析再取得や録音変更で古い候補を閉じ、再生・録音・分析世代の照合で遅延コールバックを拒否する。[聞き直し仕様](../specs/FILLER_CHAPTERS.md)、[Issue #83](https://github.com/MinobeRyo/kanpeki/issues/83)。候補生成と再生ガードはcoreの合成テストで確認し、実機の音声出力・中断からの操作一巡は別途確認する。録画・共通時計・統一タイムラインの追加ではない。
