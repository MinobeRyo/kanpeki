...
Macアプリ上部「音声分析」でモデル準備→受信開始→URLとコード表示→結果確認ができます。[Mac版の手順](MAC_AUDIO.md)を参照してください。最新の本体MacではPythonサーバーの別起動は不要です。

## 本体iPhoneで音声を試す

`KanpekiPhone` のメニュー→その他→確認・接続に「音声を試す」を配置する。同じ録音・通信・結果画面を単体アプリと本体で共有する。`KanpekiAudioApp.swift` と単体アプリの署名設定は本体ターゲットへ含めない。本体のBundle ID・署名・配布先は既存設定を維持する。

これは独立録音の試験入口で、ここから開始した録音は発表へ付け直さない。発表に関連付ける別入口は下の「部分統合」を参照。共有接続を切ったホームから開き、カメラを停止して試す。録音画面を戻る/バックグラウンドへ移すと録音は停止し、次に音声画面を開いて取得済み音声を分析できる。分析中に戻ると待機をキャンセルする。画面を開くだけではマイクを要求しない。

1. この変更を含む本体iPhoneビルドをインストールして、メニュー→その他→確認・接続→「音声を試す」を開く。TestFlight配布状態は [Issue #28](https://github.com/MinobeRyo/kanpeki/issues/28) で確認する。PR作成・マージ・ローカルビルド成功だけではTestFlight更新済みとはしない。
2. 本体Macの「音声分析」でモデルを準備して「受信を開始」。表示されたURLとコードを使う。旧Pythonサーバーは比較用として残す。
3. iPhoneとMacを同じ信頼できるWi-Fiへ接続し、Macの「音声分析」に表示されたURLと接続コードを入力する。これは音声サーバー用のコードで、本体スライド接続とは別。初版はLAN内のHTTP通信。
...
本体iPhoneのメニュー→その他→確認・接続→「発表の音声」（欄名「発表の音声 · このiPhone」）から、発表タイマーが進行中または一時停止中のときだけ明示操作で録音を開始できる。カメラ分析は開始時に停止し、録音中のカメラ入口は無効化する。録音を使わなくても発表できる。
...
録音時間は録音の時計であり、発表タイマーの時間ではない。発表中の途中開始や一時停止区間を含むことがある。Mac/iPhoneの時計対応は未接続なので、スライド履歴は空のまま送る。受信時刻を録音時刻に見せない。カメラとの同時計測、共通結果サマリーへの音声詳細・時計の同期は残件。本体Macの受信サービスとWhisper分析は [MAC_AUDIO](MAC_AUDIO.md) を参照。実機の受入・配布確認とは区別する。
...
Xcodeで `experiments/AudioCapture/ios/KanpekiAudio.xcodeproj` を開く。Bundle IDは `dev.kanpeki.audio`。この単体アプリ自体のTestFlight配布設定は未接続。本体TestFlightでは上記のメニュー内の入口を使う。
