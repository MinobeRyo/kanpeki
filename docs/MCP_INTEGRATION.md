...
利用者にどんな改善を提案するかは[AI_COACHING](../specs/AI_COACHING.md)を参照してください。#82の初期実装では、根拠・具体的な修正・次の練習での確認をそろえ、最優先の改善1件を先に表示します。正確なページ別時間や発話との時刻同期、前回比較は追加していません。
...
ホームの「分析・結果」→「カメラ・AI振り返り・その他」→「ChatGPTで発表を振り返る」、またはメニューバー「準備」→「ChatGPTで振り返る」から開きます。「共有の設定」で共有を開始/停止し、発表後は同じ画面で依頼文のコピーと結果確認を行います。


## #82：具体的な改善と次の練習

今回の変更で新規の発表後分析は `schemaVersion: 2` を使います。Mac/iPhoneとMCPサーバーを同じ変更版へ更新し、サーバー再起動とChatGPTのMCPツール一覧更新を行ってから、新しい依頼を作成してください。古いMCPは新しい依頼を拒否するため、そのまま待ち続けないでください。旧schema 1の依頼と文章のみの結果の読み取りは互換維持しています。

### 取得情報から作る確認候補

- 音声の各区間には任意の `audioRange: {recordingID, startSeconds, endSeconds}` を付けます。秒数は同じ録音開始からの相対時刻。空区間には付けません。
- `get_practice_report(source: analysis)` は固定したfactsに加え `coachingContext` を返します。同じ録音内で重なるフィラー/低音量候補と認識文のIDをまとめ、確認区間を最大3件に絞ります。元のfactsと依頼IDは変更しません。
- これは失敗や原因の検出ではありません。認識区間の時刻は単語の正確な発音時刻ではなく、発表/カメラ時計との同期も示しません。区間情報がなければ推測で補いません。
- `timer.comparison` はMacが計算する実測経過−設定時間の符号付き差です。設定なし・非有限値では作りません。ページごとの配分や修正による時間短縮を表す値ではありません。

### 返却形式

各項目は既存の `kind / text / evidenceIDs` を持ち、schema 2の `improvement` には次の `coaching` を付けます。

| 項目 | 内容と検証 |
|---|---|
| `targetEvidenceID` | 同じ項目のevidenceIDsの一つ。実在する資料本文、録音内の認識/計測区間、実測時間が対象。statusや未共有だけの根拠は対象にしない |
| `change` | 対象の具体的な修正案。空白だけは不可、UTF-16で600単位以内 |
| `rehearsal` | 次の練習で確認する行動。同じ長さ制限 |

改善は最大3件を優先順に返します。良かった点と限界を含め全体は1〜8件。根拠不足ならlimitationだけでよく、改善の生成は強制しません。MCPとSwiftで依頼・発表・対象・根拠・件数・必須項目を検証します。根拠の引用文はアプリが固定依頼から復元します。

Mac/iPhoneは最初の改善の説明・修正案・次の練習を先に表示し、根拠とほかの提案は折りたたみます。旧結果は文章と根拠を表示し、未取得の修正案欄を埋めません。原稿の自動上書き、録音の自動開始、前回との効果比較は追加していません。

この契約検証は助言の意味や有効性を保証しません。実ChatGPTの新ツール返却と助言品質、実録音/実カメラ、実機画面・通信・TestFlightは別途確認が必要です。今回の自動試験・ビルド結果はIssue #82と実装PRへ記録します。

### #82のローカル検証結果（2026-09-15）

- MCP 65テスト成功、Swiftの分析契約88チェック成功、既存core成功。
- Swift生成のschema 2依頼→実MCP SDK→結果返却→Swiftの対象/根拠復元まで成功（往復時89チェック）。
- Mac universal / iPhone Simulatorの署名なしDebugビルド成功。初回は実行環境のキャッシュ権限、並列時は固定依存取得のキャッシュ競合で停止したため、権限確認後にMacを単独で再実行した。製品コードで依存や検証を省略していない。
- 実コンポーネントを合成データで描画し、390/700ptの構造化・限界のみ・旧形式の6パターンで文字欠け・空欄・優先表示を確認。これは実アプリ操作や実機画面の検証ではない。
- 既存の通知MainActor警告は残る。実ChatGPTでの助言品質と返却、実機・会場入力・配布は未検証。[実装PR #91](https://github.com/MinobeRyo/kanpeki/pull/91)でmain統合とCIを確認する。
