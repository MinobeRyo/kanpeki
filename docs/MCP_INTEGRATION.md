...
元の準備アプリは別ターゲットとして取り込まれています。Mac本体の共通ホームへの画面埋め込みと、分析結果のiPhone画面への表示は未実装です。音声・カメラの指標共有と発表全体の振り返りは下記の追加統合で対応しました。未取得値は未計測のまま扱います。iPhone実機、CPU/メモリ削減、長時間連続運転、TestFlightでの統合動作は別途確認が必要です。
