...
Kanpeki.xcworkspaceをXcodeで開き、KanpekiMacまたはKanpekiPhoneで起動する。Macは左の補助領域の「カメラ分析」、iPhoneは上部のカメラアイコンから準備画面を開く。撮影対象を確認して開始し、「発表画面に戻る」で元のスライド画面へ戻る。分析なしでも元の画面に進める。カメラ解析の終了は詳細パネルから操作する。
...
カメラの追加はスライド・原稿・ページ送りの既存実装を変更しない。UI全体の少ない色・下部スライド配置はIssue #24、ポインターはIssue #16の作業と統合していく。撮影対象と結果は端末内のみで、Shared/Models.swiftの通信契約に追加していない。接続相手への集計同期は後続の範囲。
