...
extension AudioReport {
    func analysisFacts(recordingID: UUID) throws -> [PracticeFact] {
        ...
        func add(_ id: String, _ text: String) { facts.append(PracticeFact(id: "\(prefix).\(id)", kind: "audio", text: text)) }
        ...
        for (i, s) in (transcript ?? []).enumerated() { add("transcript.\(i)", "録音 \(s.start)〜\(s.end)秒: \(s.text)") }
        for (i, s) in (pace ?? []).enumerated() { add("pace.\(i)", "録音 \(s.start)〜\(s.end)秒: 認識文字数/分の推定値 \(s.charactersPerMinute)") }
        for (i, s) in quietIntervals.enumerated() { add("quiet.\(i)", "録音 \(s.start)〜\(s.end)秒: 低音量 \(s.duration)秒") }
        for (i, s) in (fillerCandidates ?? []).enumerated() {
            add("filler.\(i)", "候補「\(s.text)」、文脈「\(s.context)」、録音 \(s.start)〜\(s.end)秒（認識文の区間）。ページ: \(s.slide.map { String($0) } ?? "不明")")
        }
        if slides.isEmpty { add("slides", "音声とスライド履歴の同期は未計測です。文字起こしからページ時刻を推定しないでください。") }
        for (i, s) in slides.enumerated() { add("slide.\(i)", "録音 \(s.start)〜\(s.end)秒: ページ\(s.slide)、\(s.duration)秒") }
