...
struct PhoneScreen: View {
                ...
                .sheet(isPresented: $showDetails, onDismiss: {
                    ...
                    NavigationStack {
                        List {
                            ...
                            Section("ChatGPT") {
                                Text(model.state.analysisSharingID == nil ? "Macで分析共有を開始すると連携できます。" : "この発表の音声認識結果とカメラ集計値をMac経由でChatGPTに共有中です。")
    ...
    private var presentationAudioSection: some View {
        Section("発表の音声 · このiPhone") {
            ...
            Text("スライド時刻との同期は未接続です。MacでChatGPT共有を開始すると、この発表の認識結果を渡せます。")
                .font(.footnote)
        }
    }
